import { Game, HttpError, Player } from "../classes";
...
describe("class Game play function", () => {
    ...
    it("should call fire function of the current player BoardGame only", () => {
        ...
        firstPlayer.getBoardGame().setBoard(board1)
        secondPlayer.getBoardGame().setBoard(board2)
    ...
    it("should throw an error if the player shoot at the same place twice and let the player playe again", () => {
        const game = new Game(gameName, firstPlayer);
        game.addSecondPlayer(secondPlayer);
        const firstBoard = createEmptyGrid("-", 8);
        const secondBoard = createEmptyGrid("-", 8);
        ...
        game.play(firstPlayer, { x: 1, y: 1 });
        game.play(secondPlayer, { x: 1, y: 1 });
        expect(() => game.play(firstPlayer, { x: 1, y: 1 })).toThrow(HttpError);
        expect(game.getIsFirstPlayerTurn()).toBeTruthy()
    });
});
